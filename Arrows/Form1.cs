...
namespace SectioningProblem
{
    public partial class Form1 : Form
    {
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            _case = new CaseArgs(Case.BaseCase);
            _nozzle = new Nozzle(_case);
        ...
        private void AddObjects()
        {
            ...
            var cutPlane = Plane.XZ;
            if(_case.HillsidePlane != Plane.XY && !_case.Hillside.Equals(0.0))
                cutPlane.Translate(0, _nozzle.Center.Y, 0);

            /*
             * Add our objects to the drawing
             */

            var colors = new Color[] { Color.DarkSlateBlue, Color.CornflowerBlue, Color.CadetBlue, Color.Goldenrod };

            var objs = new List<Brep>();
            var nonWelds = new Brep[] {_nozzle.Shell, _nozzle.Neck, _nozzle.Pad };
            ...
            {
                var cut = objs[i].ConvertToMesh(0.01, 0.5);
                cut.CutBy(cutPlane, true);
                cutObjs.Add(cut);
            }
        ...
        private void _selectedCase(Object o, EventArgs args)
        {
            ...
            if (button != null && button.Tag is Case selectedCase)
            {
                ...
                try
                {
                    _case = new CaseArgs(selectedCase);
                    _nozzle.RunCase(_case);
                    AddObjects();
                    slice.Text = "See Cut";
                }
                catch (Exception e)
